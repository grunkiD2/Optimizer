...
public class BottleneckDetectorService : IBottleneckDetectorService
{
    public async Task<BottleneckReport> DetectAsync(IProgress<string>? progress = null)
    {
        ...
        var sample1 = await SampleProcessesAsync();

        await Task.Delay(30_000);

        progress?.Report("Analysing second sample...");
        var sample2 = await SampleProcessesAsync();
    ...
    private static async Task<List<Sample>> SampleProcessesAsync()
    {
        // Compute CPU % using a short 200 ms window for each process would be too slow
        ...
        // Brief async delay so counters can accumulate
